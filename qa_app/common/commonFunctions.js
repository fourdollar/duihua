...
function nlClassifierSetting(){
  ...
  console.log(credentials_NLC);
  nlClassifier = watson.natural_language_classifier(credentials_NLC);  // Create the service wrrouterer
};
...
function executeSQL(sql,callback,try_num){
  if (process.env.DEBUG == '1') console.log('executeSQL:');
  if (try_num == undefined) try_num = 1;
  try {
    ibmdb.open(constr, function (err,conn) {
      if (process.env.DEBUG == '1') console.log('executeSQL try_num : ' + try_num);
      if (process.env.DEBUG == '1') console.log(sql);
      if (err) {
        ...
        if (try_num <= 5) {
          ...
          callback(err);
          console.log("executeSQL: Open fail" + err);
        }
      } else {
        conn.query(sql, function (err, sqldata) {
          if (err) callback(err);
          else callback(err, sqldata);
          conn.close(function () {
            if (process.env.DEBUG == '1') console.log('executeSQL : done');
          });
        });
      }
    });
    ...
    if (try_num <= 5) {
      ...
      callback(err);
      console.log("executeSQL: SQL fail" + err);
    }
  }
};
exports.executeSQL = function(sql,callback){
